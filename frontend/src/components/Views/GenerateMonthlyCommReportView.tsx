...
export function GenerateMonthlyCommReportView() {
  ...
  const handleDeleteAdjustment = async (adjustmentId: string) => {
    ...
    const confirmed = confirm('Are you sure you want to delete this adjustment? This action cannot be undone.');
    if (!confirmed) return;

    try {
      const response = await fetch(`/api/commission/delete-adjustment/${adjustmentId}`, {
        method: 'DELETE',
      });
      ...
      if (response.ok) {
        ...
        const errorData = await response.json().catch(() => ({}));
        const errorMessage = errorData.message || 'Failed to delete adjustment';
        alert(`Error: ${errorMessage}`);
      }
    } catch (error) {
      console.error('Error deleting adjustment:', error);
      alert('Network error: Unable to delete adjustment. Please check your connection and try again.');
    }
  };
